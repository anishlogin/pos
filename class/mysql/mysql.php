...
Class mysql{
    ...
    static function getInstance($config=null) {
        ...
        if ($config == null && $firstInstance != null) {
            return $firstInstance;
        }

        $connection = 'mysqli';
        if ($config != null) {
            $connection = serialize($config);
        }

        $connection = md5($connection);

        if (!isset($instance[$connection])) {
            $instance[$connection] = new self($config);
        }
    ...
    function connect(){
        ...
        if(!($this->link)){
            ...
            mysqli_query($this->link, "SET time_zone = '+5:30'");
            $this->connected = true;
        }
     ...
     * @param array|bool $params any array pair of parameters
     * @return Object|bool|mysql_result OBJ_mysql_result() or false if the query failed
    ...
    function queryUniqueRecord($sql="",$params=false) {
        if(strlen($sql)!=0) {
            $sql = $sql . ' LIMIT 1';
        }
    ...
    */
    ...
    function escape($str){
        ...
        $str = function_exists("mysqli_real_escape_string") ? mysqli_real_escape_string($this->link,$str) : mysqli_escape_string($this->link,$str);
        return (string)$str;
    }
     ...
     * @return array|bool MySQL Errors
     */
    function errors(){
