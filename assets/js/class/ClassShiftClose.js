class ShiftClose{
    ...
    calcDiff(remitted,element){
        ...
        var adjusted = element.closest('tr').find('.adjusted').val()
        var diff =  parseFloat(self.bills[id].cashsales)  - parseFloat(adjusted)  - parseFloat(remitted)
    ...
    closeShifts(bills){
        ...
        $.post(url, {bills: bills,user:$('input[name="user"]').val()}, function (data) {
            ...
            if(data['success']){
                ...
                $.alert('There are no pending bills to close')
            }
    ...
    showAdjustmentDetails(id,element){
        ...
        if(self.bills[id]['remittedTotal'] > 0){
            $('#adjustmentDetailsPopup').find('input').attr('readonly',true);
        }
        self.addNewRow();
        if(self.bills[id]['remittedTotal'] > 0){
            $('#AdjustmentBody').find('tr').last().find('input').attr('disabled',true);
        }
    }
    addAdjustmentDetails(id,data,total){
        var self =this;
        console.log(self)
        self.bills[id]['adjustments'] = data;
        self.bills[id]['adjusted'] = total.toFixed(2);
        self.bills[id]['total'] = parseFloat(self.bills[id]['cardsales']) + parseFloat(self.bills[id]['cashsales']) - total;
        self.setTableData(id);
    }
    addNewRow(){
        var self =this;
    ...
    setTableData(id){
        var self =this;
        var tr = $('#listBills').find('tr[data-id="'+id+'"]');
        tr.find('.adjusted').val(self.bills[id].adjusted);
        tr.find('.full').val(self.bills[id].total);
        tr.find('.remitted').trigger('keyup');
    }
}