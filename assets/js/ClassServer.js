class ServerFunctions extends DomFunctions{
	...
	getProductByCode(code,element){
		...
		$.get(this.url, {code: code}, function (data) {
            ...
            if (data['success']) {
                ...
                id = details.id;
                console.log(details)
                details.disc_price = parseFloat(details.mrp).toFixed(2);
                details.price = parseFloat(details.mrp).toFixed(2);
	          ...
	          var template_item_bill = Template7.compile($('#template-item-bill').html());
						...
						var item_bill = template_item_bill({items:self.bill_items,
                                              ...
                                              cgst:self.cgst,
          	                                  ...
          	                                  cess:parseFloat(self.cess).toFixed(2),
  ...
  loadReturnBill(){ 
    var self  = this; 
    self.returnBill = {}
    self.url = `DataResponse.php?req=loadReturnBill`;
    var billno=$('#returnbillno').val()
    var counter=$('#returncounterno').val()
    var date=$('#returnbilldate').val()
    $.post(self.url,{counter:counter,billno:billno,date:date},function(data){ 
          if (data['success']) {
            var serial = 1;
            self.returnBill[data.counters.posid] = data.counters
            var template_counter = Template7.compile($('#Template-Return-Bill').html());
            if(data['counters']['items'] != undefined) {
                $.each(data['counters']['items'],function(i,v){
                  v.serial = serial;
                  v.posid = data['counters']['posid']
                  v.returnAmount = 0;
                  serial++;
                })
            }
            var counter_content  = template_counter({items:data['counters']['items'],pos:data['counters']});
            $('#returnItemList').html(counter_content)
            $('.ret_netTotal').val(data['counters']['BillAmount'])
            $('.returnButton').attr('data-posid',data['counters']['posid'])
          }
          else{
            $('#returnItemList').html("");
            $('.ret_netTotal').val("");
            $('.amount_torepay').val("");
            $('.returnButton').attr('data-posid',"")
            $.confirm({
              escapeKey :true,
              backgroundDismiss: true,
                  title: 'Oops!',
  ...
  processReturn(){ 
    var self = this
    self.url = `DataResponse.php?req=processReturn`;   
    var returnobj = self.returnBill;
    var returnBill = Object.keys(returnobj).map(function (key) { return returnobj[key]; }); 
    var settlecounterno=$('#settlecounterno').val();
    //alert(settlecounterno);
     console.log(returnBill);
     console.log(self);
    // return false;
    $.post(self.url,{returndata:returnBill,repayAmount:self.returnAmount,operator:self.operator,counter:self.counter,settlecounterno:settlecounterno},function(data){ 
          ...
          if (data['success']) {
            $.confirm({
              ...
              backgroundDismiss: true,
                title: 'Message',
                content: '<h8>Return Successfully</h8>',
                buttons: {
                    okay:{
                        action:function(){
                            location.href="pos.php";
