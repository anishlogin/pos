class DomFunctions {
    ...
    initializePendingBill(posid,posDetails,type='pending'){
      ...
      self.cess = bill.CESS ? parseFloat(bill.CESS) : this.cess;
        ...
        $.each(self.bill_items,function(i,v){
          ...
          $('#ItemListBody tr').each(function(n,m){
            ...
            if (!$(m).closest('tr').attr('data-id')) {
              ...
              $(m).closest('tr').children(':nth-child(6)').children(':nth-child(1)').val(v.qty)

              $(m).closest('tr').children(':nth-child(7)').html(parseFloat(v.total).toFixed(2))
              return false;
            }
          })
        })
    ...
    unsetAllRow(){
       ...
       $('#ItemListBody tr').closest('tr').children(':nth-child(5)').html('00.00')
    	 ...
    	 $('#ItemListBody tr').closest('tr').children(':nth-child(6)').children(':nth-child(1)').val('')

       $('#ItemListBody tr').closest('tr').children(':nth-child(7)').html('00.00')
    ...
    unsetRow(element){
      ...
      closeTR.children(':nth-child(6)').children(':nth-child(1)').val('')

      closeTR.children(':nth-child(7)').html('00.00')
    ...
    addProduct(product,element,id){
      ...
      element.closest('tr').children(':nth-child(4)').html(product.unit)
    	element.closest('tr').children(':nth-child(5)').html(product.disc_price)

    	element.closest('tr').children(':nth-child(6)').children(':nth-child(1)').val('')
    ...
    UpdateExistingQuantity(element,code,qty){
       ...
       if(check){	       	
          ...
          $('#ItemListBody tr[data-id="'+id+'"]').children(':nth-child(6)').children(':nth-child(1)').focus()
          ...
          if (id && id != 0) {
              ...
              self.bill_items[id] = self.items[id];
	            ...
	            self.addQuantity(qty,element,id)
    ...
    addQuantity(qty,element,id){
        ...
        self.bill_items[id] = self.items[id]
    	    ...
    	    element.closest('tr').children(':nth-child(6)').children(':nth-child(1)').val(qty)
    ...
    calcProductTotal(qty,price,element,id){
    	...
    	let self = this;
      console.log(self.items[id].disc_price)
      console.log(qty)
       var productTotal = qty*parseFloat(self.items[id].disc_price);

    	self.items[id].total = productTotal;

        element.closest('tr').children(':nth-child(7)').html(parseFloat(productTotal).toFixed(2))
    ...
    calcSubTotal(){
      ...
      $.each(self.bill_items,function(i,v){
        self.subTotal += parseFloat(v.disc_price) *parseFloat(v.qty)

        subTotal = parseFloat(self.subTotal).toFixed(2)

        let gst = parseFloat(self.gst).toFixed(2)

      	let cess = parseFloat(self.cess).toFixed(2)
    ...
    calcPayableAmount(){
      ...
      self.payablaAmt = 0;
        ...
        var payableamt = parseFloat(self.payablaAmt).toFixed(2)
    ...
    calcGrandTotal(){
      ...
      self.grandTotal += self.subTotal+self.gst+self.others+self.cess-self.disc;
    ...
    calcTaxForProduct(id,element,gst_type='inclusive'){
      ...
      let cessrate = self.items[id].cesstax;

      if (id != 0 && taxrate) {

	      if (gst_type == 'inclusive') {

	       taxableamt = (parseFloat(self.items[id].disc_price) * 100 / (100 + parseFloat(taxrate) + parseFloat(cessrate)));
        ...
        taxamt =  parseFloat(taxableamt) * (parseFloat(taxrate)/100)
	      cessamt =  parseFloat(taxableamt) * (parseFloat(cessrate)/100)
	      price = parseFloat(taxableamt)+parseFloat(taxamt)+parseFloat(cessamt);

	      self.items[id].taxableamt = taxableamt;

        self.items[id].taxamt = taxamt;

	      self.items[id].cessamt = cessamt;
    ...
    calculateAmountToRepay(element){
    	var self = this;
    	var amount = 0 ;
    	self.returnAmount = 0;
    	var Qty = element.val()
    	var posid = element.closest('tr').attr('data-posid')
    	var itemcode = element.closest('tr').attr('data-itemcode')
    	var qtypurchsed = element.closest("td").prev().html(); 
    	if(parseInt(qtypurchsed) < parseInt(Qty)) { 
    	    $.confirm({
              escapeKey :true,
              backgroundDismiss: true,
                  title: 'Oops!',
                  content: '<h8>Qty Return must be smaller than Q.Purchased</h8>',
                  buttons: {
                        okay:{
                          action:function(){
    	...
    	$.each(self.returnBill[posid]['items'],function(i,v){
    		if(v.ItemCode == itemcode){
    			if (parseInt(v.Qty) >= parseInt(Qty)) {
    				v.returnQty = parseInt(Qty)
    				amount = parseInt(v.Rate) * parseInt(Qty)
    				element.closest('tr').find('.amount_repay').text(parseInt(amount).toFixed(2))
    				v.returnAmount = amount;
    			}
    			if(Qty == '' || Qty == 'undefined'){
    				v.returnAmount = 0
    				v.returnQty = 0
    				element.closest('tr').find('.amount_repay').text(parseInt(0).toFixed(2))

    			}
    		}
    		self.returnAmount = parseInt(self.returnAmount) + parseInt(v.returnAmount) 
    	})
    	$('.amount_torepay').val(parseInt(self.returnAmount).toFixed(2))
    }
